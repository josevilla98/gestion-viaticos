...
import logocmi from "../Imagenes/Logocmi.png"; // 👈 logo
import "./Reportesuper.css";
...
const Reportesuper = () => {
  ...
  const guardarReporte = async () => {
    try {
      // 🔹 1. Obtener info del usuario desde Firestore
      ...
      // 🔹 2. Guardar reporte base en Firestore (colección de supervisores)
      const reporteRef = await addDoc(collection(db, "ReportesSupervisores"), {
        fechaInicio,
        kmInicio,
        kmFinal,
        carro,
      ...
      // 🔹 3. Guardar subcolección con los días
      const diasGuardados = [];
      for (const dia of dias) {
        ...
        for (const imagen of dia.imagenes) {
          const storageRef = ref(storage, `reportesSupervisores/${reporteId}/${dia.nombre}/${imagen.name}`);
        ...
        await addDoc(collection(db, "ReportesSupervisores", reporteId, "dias"), diaData);
      }
      ...
      const pdfRef = ref(storage, `reportesSupervisores/${reporteId}/reporte.pdf`);
      await uploadBytes(pdfRef, blob);
      const pdfUrl = await getDownloadURL(pdfRef);

      // 🔹 5. Guardar el link del PDF en el documento principal
      await updateDoc(doc(db, "ReportesSupervisores", reporteId), { pdfUrl });
  ...
  return (
    <div className="reporte-container">
      ...
      <div className="reporte-header">
        ...
        <h2>Reporte de Gastos de Salida - Supervisor</h2>
      ...
      <div className="tabla-gastos">
        {dias.map((dia, i) => (
          <div key={i} className="columna-dia">
            ...
            <input type="number" value={dia.desayuno} onChange={(e) => manejarCambio(i, "desayuno", e.target.value)} />

            <label>Almuerzo:</label>
            <input type="number" value={dia.almuerzo} onChange={(e) => manejarCambio(i, "almuerzo", e.target.value)} />

            <label>Cena:</label>
            <input type="number" value={dia.cena} onChange={(e) => manejarCambio(i, "cena", e.target.value)} />

            <label>Hospedaje:</label>
            <input type="number" value={dia.hospedaje} onChange={(e) => manejarCambio(i, "hospedaje", e.target.value)} />

            <label>Otros:</label>
            <input type="number" value={dia.otros} onChange={(e) => manejarCambio(i, "otros", e.target.value)} />

            <label>Facturas (puedes subir varias):</label>
            <input type="file" accept="image/*" multiple onChange={(e) => manejarImagenes(i, e.target.files)} />
          </div>
        ))}
      </div>
